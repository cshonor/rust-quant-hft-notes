...
> **为什么要区分？** 对症下药 — **加机器**能解决的不该去重写核心；**设计硬伤**加再多资源也喂不满，甚至越加越慢。

| 类型 | 定义（笔记版） | 本质 |
|------|----------------|------|
| **负载过高** | 请求量 **超过当前容量** → 排队变长 | **容量不够**，设计也许 OK |
| **架构问题** | 设计 **本身没法扩容** 或争用严重 | **设计硬伤**，堆资源无效 |

#### 负载 — 「量大了，机器吃不消」

**定义：** 请求量超过当前处理能力，**Saturation 排队**拉长延迟。
...
行情突增，撮合引擎接到 10 万笔订单
  但当前节点每秒只能处理 1 万笔
  →  剩下 9 万笔在队列里等  —  Saturation
  →  P99 / P999 延迟飙升

特征：流量高峰过去、或扩容节点后，延迟能恢复
```

| 信号 | 工具 / 现象 |
|------|-------------|
| 队列变长 | 应用订单队列深度 ↑、`vmstat` r 列 ↑ |
| 利用率打满但 **还能扩** | `mpstat` 部分核 idle 少、横向可加节点 |
| 短时过载 | 开盘、重大数据、某标的 burst |

**典型解法：** 扩容撮合节点、削 peak、限流、按 [延迟↔吞吐权衡](#3-延迟--吞吐量--此消彼长队列在中间) 接受排队 — **不必先动核心算法**。

#### 架构 — 「设计锁死了，加核也没用」

**定义：** 结构无法随资源线性扩展；瓶颈在 **设计** 而非 **流量**。

**HFT 例子：**

```
撮合引擎核心逻辑单线程
  机器堆 100 核 CPU，实际只吃满 1 核
  →  加机器、加核都跑不满  —  架构硬伤

更糟：多线程硬上全局锁
  →  核越多锁争用越凶，延迟反而变差（Amdahl + 锁）
```

| 信号 | 现象 |
|------|------|
| CPU 利用率「一边满一边闲」 | 单线程热路径 / 全局锁 |
| **加资源后吞吐不涨反跌** | 锁、false sharing、跨 NUMA |
| 同样低负载也慢 | 与流量无关的设计问题 |

**典型解法：** 无锁 / 分片 order book、绑核、按标的分线程、减 false sharing — **改架构**，不是再买服务器。

#### 区分方法 — 一句话 + 实验

```
加机器、加核、加节点能压下去  →  负载问题（容量）
怎么加资源都没用，甚至加了更慢  →  架构问题（设计）
```

| 实验 | 负载 | 架构 |
|------|------|------|
| 流量降一半（非 peak 时段对比） | 延迟明显恢复 | 仍然慢 |
| 横向加撮合节点 | P99 改善、吞吐线性涨 | 吞吐不涨，或 tail 更差 |
| 换更强单机（同核数） | 有一定改善 | 几乎无感（仍只吃 1 核） |

**HFT 对照：**

| 场景 | 判断 | 动作 |
|------|------|------|
| 短时间流量过载，逻辑能多节点水平扩 | **负载** | 扩几个撮合节点、弹性容量 |
| 核心单线程锁死 1 核，加机器跑不满 | **架构** | 无锁化、分片、绑核重设计 |

**和术语的衔接：** 负载高时先看 **Saturation**（队列）；怀疑架构时看 **Utilization 分布**（是否只有 1 核满）+ **Bottleneck**（`perf` 是否卡在锁或单函数）。

→ 单线程 vs 多核：[Ch 6](../../chapter-06-cpus/) · 无锁 / false sharing：[Ch 5.2](../../chapter-05-applications/notes/section-5.2-应用程序性能提升技术.md) · 方法论：[2.5](./section-2.5-性能分析方法论.md)
