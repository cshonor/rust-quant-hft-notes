...
| 方式 | 说明 |
|------|------|
| **便携版** | 搜 **「QEMU 便携版」** 或从 [qemu.org/download](https://www.qemu.org/download/#windows) 下载 Windows 版 → 解压到如 `D:\DevTools\qemu\` |
| **tolset 自带** | 原书工具包里常有 `qemu-system-i386.exe`，与后续 Day 共用 |

解压后确认目录里有 **`qemu-system-i386.exe`**。可临时加入 PATH，或用完整路径调用。
