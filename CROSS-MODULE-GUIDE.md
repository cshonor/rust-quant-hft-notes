...
| **系统底层动手** | `08` | 30 天 OS / CPU |
| **C++ 网络实战** | `09` PNP / muduo | Reactor 实验骨架 |
| **网络完整栈** | `10`–`13` | UNP → TCP/IP → Rosen → DPDK |
| **低延迟工程** | `14` HFT Practice | C++ 整机实践 |
| **Rust 量化** | `15` Rust Guide | Rust 工程 |
...
```
06 Gorman → 07 TLPI（epoll / mmap / 调度）
    ↓
08 自制 OS/CPU
    ↓
09 陈硕 PNP / muduo
...
**阅读顺序：** `07` TLPI → `08` 自制 → `09` PNP → `10` UNP → `11`–`13` 网络纵深 → `14` HFT。
