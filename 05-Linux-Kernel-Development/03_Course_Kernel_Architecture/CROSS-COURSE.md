...
| VM（a08） | Ch 12、Ch 15 |
| 网络栈（a10） | 概述 + [06-Rosen](../../13-Linux-Kernel-Networking/) |
...
| 为什么 DPDK 旁路内核 | 03 宏内核路径 vs [10-DPDK](../../14-DPDK-Low-Latency-Network/) |
...
| **管道式组合** | 工具链阶段：`binutils→gcc→glibc` 流水线 | Makefile + 内核构建系统 | `epoll` + 非阻塞 fd 组合事件流 | [08-UNP](../../11-UNP-Vol1/) · [01-CSAPP Ch11](../../01-CSAPP-3rd/chapter-11-network-programming/) |
| **内核管资源 / 用户态做功能** | p11–p12 内核 vs p6–p9 用户工具分离 | e1 内核 vs 用户态测试程序 | 标准栈 [06-Rosen](../../13-Linux-Kernel-Networking/) vs [10-DPDK](../../14-DPDK-Low-Latency-Network/) | LKD Ch 5 · [a03 分离](./episode-a03-内核架构总览.md) |
| **模块可插拔** | 可选包、按需安装 | **e2 LKM**、e4 驱动 | 动态加载、`SO`、Rust `dylib`（工程层） | LKD Ch 17 · [10-HFT ch08](../../15-HFT-Low-Latency-Practice/chapter-08-超低延迟核心引擎开发.md) |

### 网络编程中的同一套思想

- **事件驱动 / Reactor**：单线程（或少线程）上组合「监听 → 读 → 解析 → 写回」，类似 `grep | awk` 的数据流，只是发生在 socket 上
- **组件化服务**：连接管理、协议解析、业务逻辑分层 — 对应「小工具组合」在服务端架构中的版本
- 精读路径：[08-UNP](../../11-UNP-Vol1/)（API）→ [06-Rosen Ch11/14](../../13-Linux-Kernel-Networking/)（内核栈）→ [10-HFT ch06 网络](../../15-HFT-Low-Latency-Practice/chapter-06-低延迟网络与协议优化.md)
