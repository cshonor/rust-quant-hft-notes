...
> **Understanding the Linux Virtual Memory Manager** · Mel Gorman · **跳过**（**x86_64 HFT 机器通常无 `ZONE_HIGHMEM`** — 读作 **32 位 / PAE 历史** + **DMA 寻址限制** 背景；**`mempool` 概念** 见 [Ch 8](./chapter-08-Slab分配器.md)）
...
→ 铺垫：[Ch 2 §4 高端内存](./chapter-02-描述物理内存.md#4-高端内存-high-memory) · [Ch 3 §5 物理↔虚拟](./chapter-03-页表管理.md#5-地址与-struct-page-的映射)
...
→ 与 [Ch 6 `GFP_ATOMIC`](./chapter-06-物理页分配.md#4-gfp-标志与进程标志-gfp--process-flags) 同类：**硬上下文不能用会睡的路径**。
...
→ 2.6 泛化为 **[Ch 8 `mempool`](./chapter-08-Slab分配器.md#内存池-mempool)** — **任何子系统** 可 **预 reserve 关键对象**。
...
| **继续** | [Ch 10 页框回收](./chapter-10-页框回收.md) |

---

## 相关章节

- 上一章：[chapter-08-Slab分配器.md](./chapter-08-Slab分配器.md)
- 下一章：[chapter-10-页框回收.md](./chapter-10-页框回收.md)
- 附录 I：[appendix-I-高端内存管理.md](./appendix-I-高端内存管理.md)
- Ch 3 PTE in HIGHMEM：[chapter-03-页表管理.md](./chapter-03-页表管理.md#高端内存中的-pte-ptes-in-high-memory)
