...
**调优直觉：** 热路径 **少 syscall、少拷贝、少阻塞**；内核旁路见 [13-DPDK](../../../13-DPDK-Low-Latency-Network/)、用户态栈。
...
**HFT：** 高频收包 → **硬中断 + NAPI + softirq** 占 CPU；`mpstat`/`perf` 里看到 `%soft` 高要往网络栈查。→ [06 内核网络 Ch 14 NAPI/RSS](../../../12-Linux-Kernel-Networking/)
...
- **isolcpus / 专用核**： housekeeping 与 hot path 分离 → [10-HFT ch05](../../../16-HFT-Low-Latency-Practice/chapter-05-操作系统内核极致调优/)
