...
| **multi-buffer** | 批量算多个 key，**降低指令依赖**、提高 IPC → [Ch3 SIMD](../chapter-03-并行计算/notes/section-4-数据并行与SIMD.md) |
...
→ 极大提升 **路由查找** 性能；表创建/查找受 [Ch4 rwlock](../chapter-04-同步互斥机制/notes/section-3-读写锁.md) 保护（读多写少）。
